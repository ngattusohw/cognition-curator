...
class User(db.Model):
    ...
    password_hash = Column(String(255), nullable=True)  # Nullable for Apple Sign In users
    apple_id = Column(String(255), unique=True, nullable=True, index=True)  # Apple Sign In identifier
    ...
    study_preferences = Column(JSONB, default=dict, nullable=False)  # Notification prefs, study goals, etc.
    ...
    mastery_rate = Column(Float, default=0.0, nullable=False)  # Cards mastered / total cards
    ...
    retention_data = relationship("RetentionData", back_populates="user", cascade="all, delete-orphan")
        ...
        if accuracy_rate is not None:
            ...
            try:
                ...
                if study_sessions is not None:
                    ...
                    total_sessions = StudySession.query.filter_by(user_id=self.id).count()

                if total_sessions > 0:
                    self.overall_accuracy_rate = (
                        (self.overall_accuracy_rate * (total_sessions - 1) + accuracy_rate) / total_sessions
                    )
                else:
    ...
    def to_dict(self, include_sensitive=False):
        ...
        data = {
            ...
            'average_session_length_minutes': round(self.average_session_length_minutes, 1),
